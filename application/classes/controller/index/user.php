...
class Controller_Index_User extends Controller_Index {
   ...
   if(!file_exists("user_{$id}")) { $errors = array(Kohana::message('auth/lab', 'no_lab'));}
...
var_dump($files);
    ...
    public function action_settings(){
        ...
        if (isset($_POST['submit'])) {
            $data = Arr::extract($_POST, array('id', 'firstname', 'lastname', 'otch', 'icq', 'skype'));
            $settings = ORM::factory('user', $data['id'])
                ->set('firstname' , $data['firstname'])
                ->set('lastname', $data['lastname'])
                ->set('otch', $data['otch'])
                ->set('icq', $data['icq'])
                ->set('skype', $data['skype'])
                ->update();
        }
    ...
    public function action_sendpm(){
        ...
        if (isset($_POST['submit'])) {
            $data = Arr::extract($_POST, array('from_id', 'to_id', 'topic', 'txt'));
            $dt = date("Y-m-d G:i:s");
    ...
    public function action_updateavatar(){
        ...
        $validate->rule('picture', array ('Upload', 'not_empty'))
            ->rule('picture', 'Upload::type', array(':value' , array('jpg', 'png', 'gif')));
        ...
        if ($validate->check())
        {
            ...
            if( Image::factory($filename)->height > 250){
                Image::factory($filename)
                    ->resize(250, 250, Image::WIDTH)
                    ->save('./images/avatars/' . $newfile . '.jpg');

                ORM::factory('user', Auth::instance()->get_user())
                    ->set('avatar', $newfile)
                    ->save();
            }
