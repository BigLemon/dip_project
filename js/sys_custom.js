...
jQuery(document).ready(function(){
    ...
    jQuery("#register").leanModal({ top : 200, overlay : 0.4, closeButton: ".modal_close" });

    var male = [ "Алексей", "Петя", "Гоша", "Дима" ];
    var female = [ "Маша", "Оля", "Даша", "Катя" ];
    $('#firstname').change(function(){
        var firstname = $('#firstname').val();
        var sex = $.inArray(firstname, male);
        if(sex > -1) {
            $("#optionsRadios1").attr("checked", "checked");
        }
        else {
            sex = $.inArray(firstname, female);
            if(sex > -1) {
